...
function EmptyBoxState({onSelectOption}: any) {
    return (
        <div className='mt-7'>
            ...
            <p className='text-center text-gray-400 mt-2'>Discover new places now!</p>
        
        <div className='flex flex-col gap-5 mt-7'>
                        {suggestions.map((suggestions, index) => (
                            <div key={index} 
                            onClick={()=>onSelectOption(suggestions.title)}
                            className='flex items-center gap-2 border rounded-xl p-5 cursor-pointer hover:border-primary hover:text-primary'>
                                {suggestions.icon}
